...
function FerriteAssembly.element_routine!(Ke, re, state, ae, material::LinearElastic, cv::CellVectorValues, buffer)
...
function FerriteAssembly.element_residual!(re, state, ae, material::LinearElastic, cv::CellVectorValues, buffer)
    for q_point in 1:getnquadpoints(cv)
        dΩ = getdetJdV(cv, q_point)
        ϵ = function_symmetric_gradient(cv, q_point, ae)
        σ = material.C ⊡ ϵ
