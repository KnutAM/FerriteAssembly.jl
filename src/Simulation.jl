...
get_num_tasks(sim::Simulation) = get_num_tasks(get_domainbuffer(sim))
get_chunks(sim::Simulation{<:AbstractDomainBuffer}) = get_chunks(get_domainbuffer(sim))
get_itembuffer(sim::Simulation, args::Vararg{Any, N}) where {N} = get_itembuffer(get_domainbuffer(sim), args...)
