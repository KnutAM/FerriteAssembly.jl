...
"""
function work!(worker, multisim::MultiDomainSim)
    for (name, sim) in multisim
        skip_this_domain(worker, name) && continue
        work_domain_sequential!(worker, sim)
    end
end
function work!(worker, sim::SingleDomainSim)
    work_domain_sequential!(worker, sim)
end
function work!(worker, multisim::MultiDomainThreadedSim)
...
function work!(worker, sim::SingleDomainThreadedSim)
    if can_thread(worker)
...
function work_domain_sequential!(worker, sim::Simulation{<:AbstractDomainBuffer})
...
function work_domain_threaded!(workers, sim::SingleDomainThreadedSim)
