@testset "heatequation" begin
    ...
    function setup_assembly_test(dh, material, cv; autodiff_cb=false, threaded=false)
        ...
        if isa(material, Dict) && isa(dh, DofHandler)
            ...
            ad1 = GridDomain(dh, material["A"], cv; set=setA)
            ad2 = GridDomain(dh, material["B"], cv; set=setB)
        ...
        elseif isa(material, Dict) && isa(dh, MixedDofHandler)
            ...
            ad1 = GridDomain(sdh1, material["A"], cv; set=intersect(setA, set1)) # sdh1A
            ad2 = GridDomain(sdh1, material["B"], cv; set=intersect(setB, set1)) # sdh1B
            # For ad3 and ad4; add the full set to check correct intersection with sdh2's cellset internally. 
            ad3 = GridDomain(sdh2, material["A"], cv; set=setA) # sdh2A 
            ad4 = GridDomain(sdh2, material["B"], cv; set=setB) # sdh2B
        ...
        elseif isa(dh, MixedDofHandler)
            ...
            ad1 = GridDomain(sdh1, material, cv; set=set1)
            ad2 = GridDomain(sdh2, material, cv; set=set2)
            ...
            buffer = setup_domainbuffer(GridDomain(dh, material, cv); autodiffbuffer=autodiff_cb, threading=threaded)
